    ...
    default OutputStream asOutputStream() {
        return new OutputStream() {
            ...
            public void write(int b) throws IOException {
                int prev = checkState();
                try {
                    state = Utf8.nextState(prev, (byte)b, Utf8ByteHandler.this);
                } catch (Exception x) {
            ...
            public void write(byte[] b, int off, int len) throws IOException {
                int prev = checkState();
                try {
                    state = Utf8.nextState(prev, b, off, len, Utf8ByteHandler.this);
                } catch (Exception x) {
                    throw x instanceof IOException ? (IOException)x : new IOException(x);
                }
            }
            ...
            public void close() throws IOException {
                int prev = checkState();
                try {
                    Utf8.transferFinalState(prev, Utf8ByteHandler.this);
